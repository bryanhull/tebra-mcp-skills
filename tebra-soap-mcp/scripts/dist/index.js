...
function extractDiagnostics(parsed) {
    ...
    const walk = (node) => {
        ...
        for (const [key, value] of Object.entries(obj)) {
            if (key === "SecurityResponse")
                diagnostics.securityResponse = value;
            if (key === "ErrorResponse")
                diagnostics.errorResponse = value;
            if (key === "s:Fault")
                diagnostics.fault = value;
            if (key === "CustomerKeyValid")
                diagnostics.customerKeyValid = value;
            if (key === "Authenticated")
                diagnostics.authenticated = value;
            if (key === "Authorized")
                diagnostics.authorized = value;
            if (key === "CustomerId")
                diagnostics.customerId = value;
            walk(value);
        }
    };
    walk(parsed);
...
server.registerTool("tebra_soap_list_operations", {
...
server.registerTool("tebra_soap_debug_auth", {
    title: "Debug SOAP Auth",
    description: "Run auth diagnostics against SOAP and extract SecurityResponse/ErrorResponse/fault details for support troubleshooting.",
    inputSchema: {
        operations: z
            .array(z.string())
            .optional()
    ...
    for (const op of ops) {
        try {
            const out = op === "GetCustomerIdFromKey"
...
server.registerTool("tebra_soap_health_check", {
...
server.registerTool("tebra_soap_call_operation", {
    title: "Call SOAP Operation (Generic)",
    description: "Call any SOAP operation. RequestHeader is added automatically; provide only operation-specific XML fields.",
    inputSchema: {
        operation: z
            .string()
            .min(1)
        ...
        extra_request_xml: z
            .string()
            .optional()
            .describe("Raw XML inserted under the request element, after RequestHeader (for example: <api:PatientID>123</api:PatientID>)")
    },
    annotations: {
        readOnlyHint: false,
        destructiveHint: false,
...
server.registerTool("tebra_soap_get_patient", {
    title: "Get Patient",
...
server.registerTool("tebra_soap_get_appointments", {
...
server.registerTool("tebra_soap_get_patients", {
    title: "Get Patients",
...
server.registerTool("tebra_soap_get_charges", {
    title: "Get Charges",
...
server.registerTool("tebra_soap_get_payments", {
    title: "Get Payments",
...
main().catch((error) => {
    process.stderr.write(`Tebra SOAP MCP server failed to start: ${error instanceof Error ? error.stack ?? error.message : String(error)}\n`);
    process.exit(1);
});
