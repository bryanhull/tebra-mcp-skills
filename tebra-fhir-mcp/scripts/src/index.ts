...
server.registerTool(
  "tebra_search_patients",
  {
    ...
    description:
      "Search patients by id, identifier, name, birthdate, or gender using Tebra FHIR Patient endpoint.",
    inputSchema: {
...
server.registerTool(
  "tebra_search_resource",
  {
    ...
    description:
      "Search a Tebra FHIR resource by name with optional patient and additional query params.",
    inputSchema: {
      resource: z
        .string()
        .min(1)
        .describe(
          "FHIR resource name such as AllergyIntolerance, Condition, Observation, DiagnosticReport, Encounter"
        ),
...
server.registerTool(
  "tebra_patient_clinical_summary",
  {
    ...
    description:
      "Fetch a compact patient summary bundle by querying common USCDI clinical resources for one patient.",
    inputSchema: {
      patient_id: z.string().min(1).describe("FHIR patient id"),
      resources: z
        .array(z.string())
        .optional()
        .describe(
          "Optional list of resources. Defaults to AllergyIntolerance, Condition, MedicationRequest, Observation, DiagnosticReport, Encounter, Procedure, DocumentReference."
        ),
  ...
  async ({ patient_id, resources, count_per_resource }) => {
    ...
    for (const resource of selected) {
      try {
        ...
        (summary.bundles as Record<string, unknown>)[resource] = bundle;
      } catch (error) {
        (summary.bundles as Record<string, unknown>)[resource] = {
...
main().catch((error) => {
  process.stderr.write(
    `Tebra FHIR MCP server failed to start: ${error instanceof Error ? error.stack ?? error.message : String(error)}\n`
  );
  process.exit(1);
});
